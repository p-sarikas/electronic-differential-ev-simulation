...
slope_m                  = [0;0.08;0;0;0.02;]; % 10% slope -> 5.74deg 
...
%% PID
Kp = 1.37059949740112;   % Proportional
Ki = 22.6375234952304;   % Integral
Kd = 0.0180275940261083; % Derivative
Kn = 2793.84392001139;   % Filter coefficient

output_saturation_upper = 400;
output_saturation_lower = -400;
...
n_eff_gear = 0.98;   % [-] Efficiency of the gearbox 

%% Speed Reference
% w_ref_initial = 400;        % [rad/s] (initial position) the accelerator pedal command sets the common reference speed
% w_ref_1       = 500;        % [rad/s] (position 1) the accelerator pedal command sets the common reference speed
% time_w_ref_1  = 5;          % [sec] (position 1) start time of position 1
% w_ref_2       = 615.3846;   % [rad/s] (position 2) the accelerator pedal command sets the common reference speed
...
Field_DC_voltage_source_1 = 120;  % [Volt] DC Field voltage source for motor 1
Field_DC_voltage_source_2 = 120;  % [Volt] DC Field voltage source for motor 2
motor_power_hp = 40;        % 30(8) 50(12)
motor_power_w  = motor_power_hp*745.7
total_motor_power_w = motor_power_w*2;
% wheel_kph = w_ref*Rw*3.6/i_gear
% wheel_kph = 80;
% w_ref_req = wheel_kph/ (Rw*3.6/i_gear)

% sim('Electronic_Differential_PID_DC_motor_01_Simulink_Model')
